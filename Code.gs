...
/**
 * HƯỚNG DẪN SỬ DỤNG GOOGLE APPS SCRIPT VỚI GOOGLE SHEET
 *
 * 1. Mở Google Sheets:
 *    - Trước hết, hãy tạo hoặc mở một Google Sheet mới mà bạn muốn sử dụng để lưu trữ thông tin file từ Google Drive.
 *
 * 2. Mở Google Apps Script:
 *    - Từ Google Sheet, nhấp vào "Extensions" (Tiện ích mở rộng).
 *    - Chọn "Apps Script". Một cửa sổ trình chỉnh sửa Google Apps Script sẽ được mở.
 *
 * 3. Thêm Code Apps Script:
 *    - Trong trình chỉnh sửa Apps Script, bạn sẽ thấy một file tên là "Code.gs".
 *    - Xóa tất cả các mã mặc định trong file này.
 *    - Sao chép và dán hàm listFilesInFolderBatch() vào trình chỉnh sửa mã.
 *
 * 4. Thiết lập:
 *    - Đảm bảo rằng bạn thay thế "folderId" bằng ID của thư mục Google Drive mà bạn muốn lấy thông tin.
 *    - ID của thư mục nằm sau "/folders/" trong đường dẫn URL của thư mục Google Drive.
 *    - Nhấn "Save" hoặc nhấn Cmd + S (macOS) hoặc Ctrl + S (Windows) để lưu lại mã.
 *
 * 5. Chạy Hàm:
 *    - Chọn hàm bạn muốn chạy từ thanh công cụ (ví dụ: listFilesInFolder hoặc listFilesInFolderBatch).
 *    - Nhấn nút "Run" (Chạy) (biểu tượng tam giác giống như "Play").
 *    - Lưu ý: Nếu đây là lần đầu tiên bạn chạy mã, bạn sẽ được yêu cầu cấp quyền truy cập vào tài khoản của mình.
 *      Nhấp vào "Review Permissions" (Xem xét quyền) và "Allow" (Cho phép).
 *
 * 6. Kiểm tra Google Sheet:
 *    - Sau khi chạy xong hàm, bạn có thể quay lại Google Sheet và kiểm tra kết quả.
 *    - Google Sheet sẽ chứa thông tin về tất cả các file từ thư mục Google Drive mà bạn đã chỉ định,
 *      bao gồm tên file, loại, kích thước, liên kết chia sẻ, và các thông tin khác.
 *
 * 7. Lưu ý khi sử dụng:
 *    - Quota và thời gian thực thi:
 *      + (Lỗi Exceed time execution) Google Apps Script có thời gian thực thi tối đa là 6 phút cho mỗi lần chạy.
 *      + Nếu bạn cần xử lý nhiều file hơn (mặc định là 100 files), thì bạn cứ nhấn Run nhiều lần (sau mỗi lần hàm được thực thi xong),
 *        nhấn Run từ lần thứ 2 thì file sẽ tiếp tục được thực hiện tại vị trí dừng và tiếp tục được ghi tiếp vào gg sheets.
 *    - Giới hạn của Google Drive và Sheets:
 *      + Google Sheet có giới hạn khoảng 10 triệu ô dữ liệu, vì vậy nếu số lượng file quá lớn, bạn có thể gặp phải giới hạn về dung lượng.
 *      + Google Apps Script có một số giới hạn về tài nguyên, bao gồm số lượng yêu cầu có thể thực hiện đến Google Drive trong một ngày.
 *
 * SỬ DỤNG TRIGGER ĐỂ TỰ ĐỘNG HÓA:
 *    - Bạn có thể cài đặt Trigger để chạy hàm tự động, ví dụ:
 *      + Chạy hàng ngày hoặc hàng tuần để cập nhật danh sách file.
 *    - Để thiết lập Trigger, từ trình chỉnh sửa Apps Script, nhấp vào biểu tượng "Đồng hồ" (Triggers) ở thanh bên trái.
 *    - Nhấn "Add Trigger" và chọn hàm bạn muốn chạy, cùng với tần suất bạn muốn (ví dụ: mỗi ngày).
 */
